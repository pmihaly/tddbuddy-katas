import { DoorState, doorToString, hundredDoors, mapEveryNthElement, toggleDoor } from './hundred-doors';
...
describe('mapEveryNthElement', () => {
  it('should double every 3rd element', () => {
    ...
    expect(mapEveryNthElement(double)(3)(array)).toEqual([1, 2, 6, 4, 5, 12, 7, 8, 18, 10]);
  })
})

const produceArray = <TValue>(length: number, value: TValue): TValue[] => Array.from({ length }, () => value);
...
describe('houndredDoors', () => {
  ...
  it('should open every second door on second pass', () => {
    ...
    expect(hundredDoors(2)(doors)).toEqual(produceArray(50, <DoorState[]>['closed', 'half-open']).flat());
})
})
